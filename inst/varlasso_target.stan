...
parameters {
  ...
  vector<lower=0>[est_lambda*1] lambda2; // parameters for Student-t and laplace priors
...
transformed parameters {
  ...
  if(off_diag_priors == 1) {
     //sqrt_lambda2 = sigma_scale * sqrt(lambda2);
     //Boffd = sqrt_lambda2 .* B_z;
     //Student t priors
     //for(i in 1:n_off) {
...
model {
  ...
  target += student_t_lpdf(sigma_proc | 3.0, sigma_proc_mu, sigma_proc_sd) -
  1* student_t_lpdf(0 | 3.0, sigma_proc_mu, sigma_proc_sd);

  // obs SD
  //sigma_obs ~ student_t(3, sigma_obs_mu, sigma_obs_sd);
  target += student_t_lpdf(sigma_obs | 3.0, sigma_obs_mu, sigma_obs_sd) -
  1* student_t_lpdf(0 | 3.0, sigma_obs_mu, sigma_obs_sd);

  // B diagonal
  //Bdiag ~ normal(b_mu_diag,b_sd_diag);
  target += normal_lpdf(Bdiag | b_mu_diag, b_sd_diag);
  ...
  if(off_diag_priors == 1) {
    ...
    target += student_t_lpdf(sigma_scale | sigma_scale_df,0,sigma_scale_sd);
    if(est_nu==1) {
      //nu[1] ~ gamma(2, 0.1);
      //lambda2 ~ inv_gamma(nu[1]/2, nu[1]/2);
      target += gamma_lpdf(nu[1] | 2.0, 0.1);
      target += inv_gamma_lpdf(lambda2 | nu[1]/2, nu[1]/2);
      //for(i in 1:n_off) {
      //  lambda2[i] ~ inv_gamma(nu[1]/2, nu[1]/2); // vector of local variances
        //B_z[i] ~ student_t(nu[1], 0, sigma_scale);
      //}
    } else {
      //lambda2 ~ inv_gamma(nu_known/2.0, nu_known/2.0);
      target += inv_gamma_lpdf(lambda2 | nu_known/2.0, nu_known/2.0);
      //for(i in 1:n_off) {
        //B_z[i] ~ student_t(nu_known, 0, sigma_scale);
        //lambda2[i] ~ inv_gamma(nu_known/2, nu_known/2); // vector of local variances
      //}
    }
    target += normal_lpdf(B_z | 0, sigma_scale * sqrt(lambda2));

  }
  if(off_diag_priors == 2) {
    // lambda2 here is really inv_lambda
    target += chi_square_lpdf(lambda2 | sigma_scale_df);
    target += double_exponential_lpdf(B_z | 0, sigma_scale_sd * lambda2[1]);

    //B_z ~ std_normal();
    //sigma_scale ~ student_t(sigma_scale_df,0,sigma_scale_sd);
    //B_z ~ double_exponential(0, sigma_scale);
    //lambda2 ~ exponential(100); // vector
  }
  if(off_diag_priors == 3) {
    B_z ~ std_normal();
  ...
  if(priors_only==0) {
    for(i in 1:n_pos) {
      ...
      target += normal_lpdf(yy[i] | x[species_index[i],time_index[i]], sigma_r[id_r[species_index[i]]]);
    }
  } else {
    target += normal_lpdf(dummy | 0, 1);
    //dummy ~ normal(0,1);
  }
